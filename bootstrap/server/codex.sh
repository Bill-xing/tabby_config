    ...
    awk -v selector="$selector" '
      $1 == selector {
        ...
        for (column = 2; column <= NF; column++) {
          if ($column == "not" && $(column + 1) == "installed") {
            row_missing = 1
          }
          if ($column == "installed") {
            row_installed = 1
...
install_pretty_mermaid() {
  ...
  if pretty_mermaid_valid "$target"; then
  ...
  if ! pretty_mermaid_valid "$target"; then
    warn "pretty-mermaid checkout is missing a valid SKILL.md: $target"
    return 1
  fi
