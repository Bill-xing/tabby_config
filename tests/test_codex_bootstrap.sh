...
reset_case() {
  ...
  export HOME CODEX_HOME MOCK_BIN CODEX_LOG CLONE_LOG NPM_LOG EVENT_LOG
...
PLUGIN_LIST=$'figma@openai-curated 1.0.0 installed enabled\nsuperpowers@openai-curated 1.0.0 installed disabled\ngmail@openai-curated 1.0.0 installed enabled\nnotion@openai-curated 1.0.0 installed enabled'
...
PLUGIN_LIST=$'figma@openai-curated-extra 1.0.0 installed enabled\nsuperpowers@openai-curated 1.0.0 not installed'
...
mkdir -p "$CODEX_HOME/skills/pretty-mermaid"
printf '%s\n' 'not a skill' >"$CODEX_HOME/skills/pretty-mermaid/SKILL.md"
...
pretty_mermaid_valid "$CODEX_HOME/skills/pretty-mermaid" ||
...
PLUGIN_ADD_STATUS=0
install_codex_server
assert_eq 2 "$(plugin_add_count)" 'rerun completes missing plugins safely'
